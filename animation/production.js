...
const { routers, publicPath, templatePaths } = require('./config.json')
const webpackConfig = {
...
if (templatePaths.length > 0) {
  const distPath = templatePaths.map((relativePath) => {
    return path.resolve(__dirname, relativePath)
  })
  webpackConfig.plugins.push(new AssetsReplaceWebpackPlugin(distPath))
}
